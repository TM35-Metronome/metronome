...
pub const Trainer = extern struct {
    ...
    pub fn partyMember(trainer: Trainer, party: []u8, i: usize) ?*align(1) PartyMemberBase {
        ...
        return std.mem.bytesAsValue(PartyMemberBase, party[start..end][0..@sizeOf(PartyMemberBase)]);
    }
};
...
const EncryptedStringTable = struct {
    ...
    fn getEncryptedString(
        ...
        const offset = section_offset + entry.offset;
        const res = table.data[offset..][0 .. entry.count * @sizeOf(u16)];
    ...
    fn sectionOffsets(table: EncryptedStringTable) []align(1) u32 {
        const h = table.header();
        const rest = table.data[@sizeOf(Header)..];
        return std.mem.bytesAsSlice(u32, rest[0 .. @sizeOf(u32) * h.sections]);
    }

    fn entries(table: EncryptedStringTable, section_offset: u32) []align(1) Entry {
        const h = table.header();
        _ = std.mem.bytesAsValue(u32, table.data[section_offset..][0..@sizeOf(u32)]);
        const rest = table.data[section_offset + @sizeOf(u32) ..];
        return std.mem.bytesAsSlice(Entry, rest[0 .. @sizeOf(Entry) * h.entries]);
    }

    fn size(sections: u32, strings: u32, chars: u32) u32 {
        return @sizeOf(Header) + // Header
            @sizeOf(u32) * sections + // Section offsets
            @sizeOf(u32) * sections + // Entry counts
...
fn decrypt(data: []align(1) const u16, out: anytype) !u16 {
    const H = struct {
        fn output(out2: anytype, char: u16) !bool {
            ...
            const pair: Pair = switch (char) {
                ...
                0xfffe => .{ .len = 1, .codepoint = '\n' },
                    ...
                    .len = std.unicode.utf8CodepointSequenceLength(char) catch unreachable,
                    .codepoint = char,
                },
            };

            var buf: [4]u8 = undefined;
            _ = try std.unicode.utf8Encode(pair.codepoint, buf[0..pair.len]);
    ...
    for (data[start..], start..) |c, i| {
        const decoded = c ^ table.get(data.len, i);
        if (compressed) {
...
fn getKey(data: []align(1) const u16) u16 {
...
fn encode(data: []const u8, out: anytype) !void {
    var n: usize = 0;
    while (n < data.len) {
        if (std.mem.startsWith(u8, data[n..], "\n")) {
            try out.writeInt(u16, 0xfffe, .little);
            n += 1;
            continue;
        }
        if (std.mem.startsWith(u8, data[n..], "\\x")) {
            const hex = data[n + 2 ..][0..4];
            const parsed = try std.fmt.parseUnsigned(u16, hex, 16);
            try out.writeInt(u16, parsed, .little);
            n += 6;
            continue;
        }

        const ulen = std.unicode.utf8ByteSequenceLength(data[n]) catch unreachable;
        if (data.len < n + ulen)
            break;

        const codepoint = std.unicode.utf8Decode(data[n..][0..ulen]) catch unreachable;
...
fn encrypt(data: []align(1) u16, _key: u16) void {
    var key = _key;
    for (data) |*c| {
        c.* = c.* ^ key;
        key = (key << 3) | (key >> 13);
...
test "KeyTable" {
    var j: usize = 0;
    while (j <= std.math.maxInt(u16)) : (j += 1)
    ...
    for ([_]u16{ 0xf0f0, 0x0f0f, 0xdead, 0xbeef }) |key| {
        ...
        while (len < 40) : (len += 1) {
            var i: usize = 0;
            while (i < len) : (i += 1)
                try std.testing.expectEqual(keyForI(key, len, i), table.get(len, i));
        }
    }
}
...
pub const Game = struct {
    ...
    pub const Story = struct {
        ...
        pub fn deinit(story: Story, allocator: std.mem.Allocator) void {
    ...
    pub fn fromRom(allocator: std.mem.Allocator, nds_rom: *rom.nds.Rom) !Game {
        ...
        const story = try file_system.openNarc(rom.nds.fs.root, info.story);
        ...
        for (trainer_parties, 0..) |*party, i| {
            ...
            while (j < trainer.party_size) : (j += 1) {
                const base = trainer.partyMember(party_data, j) orelse break;
    ...
    pub fn fromRomEx(
        ...
        const hm_tms = std.mem.bytesAsSlice(u16, arm9[hm_tm_index..][0..hm_tm_len]);
    ...
    fn applyArm9(game: Game) !void {
        ...
        std.mem.writeInt(u24, &len_bytes, @as(u24, @intCast(game.owned.old_arm_len + 0x4000)), .little);
    ...
    fn applyTrainerParties(game: Game) !void {
        ...
        var builder = rom.nds.fs.SimpleNarcBuilder.init(buf, trainer_parties.len);

        // We make sure that the new narc we create always has room for a full
        // party for all all_trainers. By doing this, all future applies can always
        // be done inline, and allocations can be avoided. This also makes patches
        // generated simpler.
        for (builder.fat(), 0..) |*f, i|
            f.* = rom.nds.Range.init(full_party_size * i, full_party_size * (i + 1));
        ...
        for (trainer_parties, 0..) |party, i| {
            ...
            switch (party.type) {
                .none => {
                    for (std.mem.bytesAsSlice(PNone, rest[0..@sizeOf([6]PNone)]), party.members) |*m, p| {
                        m.* = PartyMemberNone{ .base = p.base };
                    }
                    @memset(rest[@sizeOf([6]PNone)..full_party_size], 0);
                },
                .item => {
                    for (std.mem.bytesAsSlice(PItem, rest[0..@sizeOf([6]PItem)]), party.members) |*m, p| {
                        m.* = PartyMemberItem{
                            .base = p.base,
                            .item = p.item,
                        };
                ...
                .moves => {
                    for (std.mem.bytesAsSlice(PMoves, rest[0..@sizeOf([6]PMoves)]), party.members) |*m, p| {
                        m.* = PartyMemberMoves{
                            .base = p.base,
                            .moves = p.moves,
                        };
                ...
                .both => std.mem.bytesAsValue(FullParty, rest[0..full_party_size]).* = party.members,
            }
        }
    ...
    fn applyStrings(game: Game, strings: []const StringTable, string_file: []const u8) !void {
        ...
        for (strings) |table| {
            ...
            writer.writeAll(&std.mem.toBytes(Header{
                .sections = 1,
            ...
            const start_of_section = @sizeOf(Header) + @sizeOf(u32);
            ...
            for (0..number_of_entries) |_| {
                writer.writeAll(&std.mem.toBytes(Entry{
                    .offset = 0,
                    .count = 0,
                    .unknown = 0,
                })) catch unreachable;
            ...
            const entries = std.mem.bytesAsSlice(Entry, bytes[start_of_entry_table..writer.context.pos]);
            for (entries, 0..) |*entry, j| {
                ...
                entry.offset = @intCast(start_of_str - start_of_section);
                entry.count = length_of_str;
    ...
    fn findScriptCommands(scripts: rom.nds.fs.Fs, allocator: std.mem.Allocator) !ScriptCommands {
        ...
        var given_pokemons_to_resolve = std.AutoArrayHashMap(usize, *align(1) u16).init(allocator);
        ...
        for (scripts.fat) |fat| {
            ...
            for (script.getScriptOffsets(script_data), 1..) |relative, i| {
                const position = @as(isize, @intCast(i)) * @sizeOf(u32);
                const offset = std.math.cast(usize, relative + position) orelse continue;
                    ...
                    switch (command.kind) {
                        ...
                        .jump, .when, .@"if", .call_routine => {
                            const off = switch (command.kind) {
                                ...
                                .when => command.when.offset,
                                .@"if" => command.@"if".offset,
                                .call_routine => command.call_routine.offset,
                                else => unreachable,
                            };
                            if (std.math.cast(usize, off + @as(isize, @intCast(decoder.i)))) |loc| {
                                if (loc < script_data.len and
                                    std.mem.indexOfScalar(usize, script_offsets.items, loc) == null)
            ...
            for (set_var_offsets.items) |offset| {
                ...
                switch (first.kind) {
                    .set_var_eq_val => if (given_pokemons_to_resolve.get(
                        ...
                        if (item.container != 32780 or
                            amount.set_var_eq_val.container != 32781)
                            continue;
                    ...
                    .set_var_2a => {
                        ...
                        _ = script.expectNext(&decoder, .unlock_all) orelse continue;
                        _ = script.expectNext(&decoder, .end) orelse continue;
                        if (first.set_var_2a.container != 32768 or
                            amount.set_var_2a.container != 32769 or
                            unknown.set_var_2a.container != 32770)
                            continue;
