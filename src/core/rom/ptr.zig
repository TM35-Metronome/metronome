...
pub fn RelativePointer(
    ...
    return extern struct {
        ...
        const Slice = @Type(blk: {
            var info = ptr_info;
            info.size = .slice;
            break :blk std.builtin.Type{ .pointer = info };
        });
        ...
        const ptr_sentinel = @as(*const ptr_info.child, @ptrCast(ptr_info.sentinel_ptr.?)).*;
        ...
        pub fn init(ptr: anytype, data: []const u8) Error!@This() {
            ...
            if (is_optional and ptr_is_optional and ptr == null)
                return @This(){ .inner = null_ptr };

            const i: Int = @intCast(@intFromPtr(ptr) - @intFromPtr(data.ptr));
        ...
        pub fn toPtr(ptr: @This(), data: Data) Error!Ptr {
            if (is_optional and ptr.inner == null_ptr)
                return null;

            const i = try ptr.toInt();
            if (data.len < i + child_size * @intFromBool(ptr_info.size == .one))
                return error.InvalidPointer;

            return @ptrCast(@alignCast(&data[i]));
        }
        ...
        pub fn toSlice(ptr: @This(), data: Data, len: usize) Error!SliceNoSentinel {
            ...
            const start = @intFromPtr(p) - @intFromPtr(data.ptr);
        ...
        fn toInt(ptr: @This()) Error!Int {
            return std.math.sub(Int, ptr.inner, offset) //
            catch return error.InvalidPointer;
        }
    };
}
...
pub fn RelativeSlice(
    ...
    return extern struct {
        ...
        const Ptr = @Type(std.builtin.Type{
            .pointer = .{
                .size = .many,
                ...
                .sentinel_ptr = @typeInfo(Slice).pointer.sentinel_ptr,
            },
        });
        const RPtr = RelativePointer(Ptr, Int, offset, 0);
        ...
        pub fn init(slice: Slice, data: []const u8) Error!@This() {
            ...
            const start = @intFromPtr(slice.ptr) - @intFromPtr(data.ptr);
