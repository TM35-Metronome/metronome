...
pub fn encodeEx(map: CharMap, curr: u1, reader: anytype, writer: anytype) !void {
    ...
    while (true) {
        ...
        for (map) |char| {
            std.debug.assert(char[in].len <= buf.len);
            if (!std.mem.startsWith(u8, chars, char[in]))
                continue;
        ...
        const rest = chars[best[in].len..];
        std.mem.copyForwards(u8, chars[0..rest.len], rest);
        len -= best[in].len;
    }
}

pub fn encode(map: CharMap, curr: u1, in: []const u8, writer: anytype) !void {
    var fis = std.io.fixedBufferStream(in);
...
fn testHelper(map: CharMap, curr: u1, in: []const u8, out: []const u8) !void {
    ...
    var fos = std.io.fixedBufferStream(&res);
    try encode(map, curr, in, fos.writer());
    try std.testing.expectEqualSlices(u8, out, fos.getWritten());
}
